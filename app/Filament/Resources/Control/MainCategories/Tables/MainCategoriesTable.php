...
namespace App\Filament\Resources\Control\MainCategories\Tables;
        ...
        return $table
            ...
            ->columns([
                TextColumn::make('id')
                    ->label(__('global.id'))
                    ->sortable()
                    ->state(fn ($record) => 'MCAT-'.str_pad($record->id, env('ID_PAD_LENGTH', 4), '0', STR_PAD_LEFT)),
                TextColumn::make('cat_name')->label(__('global.cat_name'))->searchable()->sortable(),
                TextColumn::make('cat_name_kh')->label(__('global.cat_name_kh'))->searchable()->sortable(),
