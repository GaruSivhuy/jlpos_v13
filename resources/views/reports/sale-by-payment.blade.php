<!DOCTYPE html>
<html lang="km">
@include('reports.partials.print-head', ['title' => $detail ? __('global.report_sale_by_payment_gateway_detail') : __('global.report_sale_by_payment_gateway')])
<body>
    ...
    @include('reports.partials.print-header', ['title' => $detail ? __('global.report_sale_by_payment_gateway_detail') : __('global.report_sale_by_payment_gateway')])
    ...
    <table class="table_1">
        ...
        @forelse ($resultsGroupBy as $group)
            ...
            @foreach ($dayGateways as $gateway)
                @php
                    $gatewayInvoices = $results
                        ->where('payment_gateway', $gateway->payment_gateway)
                        ->where('payment_date', $gateway->payment_date)
                        ->values();
                    $gatewayTotal = $gatewayInvoices->where('status', \App\Models\Invoice::PAID)->sum('total');
                    $grandTotal += $gatewayTotal;
                @endphp
                <tr>
                    <td class="content_regular txt_left txt_bold" colspan="{{ $detail ? 7 : 5 }}">
                        {{ __('global.payment_gateway') }} : {{ $gatewayInvoices->first()?->paymentGateway?->name }}
                    </td>
                </tr>

                @if ($detail)
                    @foreach ($gatewayInvoices as $invoice)
                        <tr>
                ...
                @else
                    ...
                    @foreach ($gatewayInvoices as $key => $invoice)
                        <tr>
